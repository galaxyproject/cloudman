var cloudmanIndexModule = angular.module('cloudman.index', [ 'cloudman.base', 'ui', 'ui.bootstrap',
        'ui.bootstrap.dialog', 'ui.bootstrap.alert', 'cloudman.popover', 'cloudman.popover.fstemplate' ]);

cloudmanIndexModule
        .service(
                'cmIndexDataService',
                function($http, $timeout, cmAlertService) {
                    // Server Status Cache
                    ...
                    var poll_data = function() {
                        ...
                        $http
                                .get(get_cloudman_index_update_url, {
                                    params : {
                                        l_log : _log_cursor
                                    }
                                })
                                .success(
                                        function(data) {
                                            ...
                                            if (_log_data.length > 200) {
                                                _log_data
                                                        .splice(0, _log_data.length - 200,
                                                                "The log has been truncated to keep up performance.  The <a href='/cloud/log/'>full log is available here</a>.");
                                            }
...
cloudmanIndexModule.controller('cmLoadGraphController', [
        '$scope',
        '$http',
        '$timeout',
        '$dialog',
        'cmIndexDataService',
        'cmAlertService',
        function($scope, $http, $timeout, $dialog, cmIndexDataService, cmAlertService) {
            ...
            $scope.get_node_fs_status = function(node) {
                ...
                if ((inst.nfs_data == "1") && (inst.nfs_tools == "1") && (inst.nfs_indices == "1")
                        && (inst.nfs_sge == "1")) {
                    return "allgood";
                }
                if ((inst.nfs_data == "0") && (inst.nfs_tools == "0") && (inst.nfs_indices == "0")
                        && (inst.nfs_sge == "0")) {
                    return "warning";
                }
                if ((inst.nfs_data == "-1") && (inst.nfs_tools == "-1") && (inst.nfs_indices == "-1")
            ...
            $scope.rebootInstance = function(node, reboot_url) {
                ...
                $dialog.messageBox(title, ud_html, btns).open().then(function(result) {
                    if (result == 'confirm') {
                        $http.get(reboot_url, {
                            params : {
            ...
            function getAliveTime(node) {
                ...
                if (hours >= 24) {
                    ...
                    time_string = ("0" + hours).slice(-2) + ":" + ("0" + minutes).slice(-2) + ":"
                            + ("0" + seconds).slice(-2);
                }
                return time_string;
            }
            ...
            var poll_performance_data = function() {
                ...
                $http.get(get_cloudman_status_update_url).success(function(data) {
                    ...
                    for (node_index in $scope.nodes) {
                        ...
                        // check whether currently known nodes are in the list
                        // of newly returned server nodes
                        for (instance_index in data.instances) {
                            var instance = data.instances[instance_index];
                            if (node.id == instance.id) {
                                // Yes, it's a known node, so just plot the new
                                // load value
                        ...
                        // Node no longer exists on server, Therefore, mark for
                        // removal
                        if (!node_found)
                    ...
                    var list_to_add = data.instances.filter(function(item) {
                        return !item.already_added
                    });

                    // Add any newly added nodes to our currently known node
                    // list
            ...
            var resumePerfDataService = function() {
                $timeout.cancel(_data_timeout_id); // cancel any existing
                                                    // timers
            ...
            function interpolatedAddPointList(series_list, points_list, frame_rate, max_series_length) {
                ...
                for ( var index in series_list) {
                    closure = interpolatedAddPoint(series_list[index].data, points_list[index], frame_rate,
                            max_series_length)
                    if (closure)
                        closureList.push(closure)
                }
                function animateSeries() {
                    pausePerfDataService(); // Pause data while animation is in
                                            // progress
                    var newList = [];
            ...
            // ----------------------------------------------------------------------------------
            // This function adds a new point to a series, but does so
            // incrementally, at the
            // specified frame_rate, by interpolating the values between its
            // previous point and
            // the new point. This can be used to create an animated effect in
            // flot charts (or other charts),
            // which do not support animation during point addition natively.
            //
            // Supports an optional argument max_series_length, which, if
            // defined,
            // limits the series length to this value, and also does an
            // interpolated remove
            // of the first point, creating a continuous, sliding animation
            // effect.
            //
            // This function is self-contained, and works incrementally on each
            // call. Returns
            // void when the starting point has been interpolated till the end
            // point. Returns
            // a closure otherwise, which can be invoked via a timer to create
            ...
            function interpolatedAddPoint(series, point, frame_rate, max_series_length) {
                ...
                series.push(from_point); // Push initial point (will be
                                            // interpolated
                // till to_point)
                ...
                var interpolateValues = function() {
                    ...
                    var animAdd = function() {
                        if (add_counter > frame_rate) {
                            series.pop(); // At end of animation, remove all
                                            // interpolated values
                            series.push(to_point); // Push last, accurate
                                                    // value, in case of
                                                    // rounding issues
                            return false;
                        }
                        ...
                        series.push(new_point); // Push in new interpolated
                                                // value towards the to_point
                        add_counter++;
                        return true;
                    }
                    ...
                    var animRemove = function() {
                        ...
                        series.splice(0, 1, remove_point); // Remove first
                                                            // element, and
                                                            // insert
                                                            // interpolated
                                                            // element in its
...
cloudmanIndexModule.controller('cmIndexMainActionsController', [
        '$scope',
        '$http',
        '$dialog',
        'cmIndexDataService',
        'cmAlertService',
        function($scope, $http, $dialog, cmIndexDataService, cmAlertService) {
            ...
            $scope.isRemoveNodeEnabled = function() {
                var cmstatus = cmIndexDataService.getCloudmanStatus();
                return cmstatus && cmstatus.autoscaling.use_autoscaling != true
                        && cmstatus.instance_status.requested != 0;
            }
        ...
        $dialog.messageBox(title, ud_html, btns).open().then(function(result) {
            if (result == 'confirm') {
                $http.get(delete_shared_instances_url, {
                    params : {
                        shared_instance_folder : instance.bucket,
                        snap_id : instance.snap
                    }
                }).success(function(data) {
                    $scope.refreshSharedInstances();
                });
...
cloudmanIndexModule.controller('cmClusterLogController', [ '$scope', '$http', '$dialog', 'cmIndexDataService',
        'cmAlertService', function($scope, $http, $dialog, cmIndexDataService, cmAlertService) {
