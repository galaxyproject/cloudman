var cloudmanAdminModule = angular.module('cloudman.admin', [ 'cloudman.base', 'ui', 'ui.bootstrap',
...
cloudmanAdminModule.service('cmAdminDataService', function($http, $timeout, cmAlertService) {
    ...
    var resumeDataService = function() {
        $timeout.cancel(_admin_data_timeout_id); // cancel any existing
        // timers
...
cloudmanAdminModule.controller('ServiceController', [ '$scope', '$http', 'cmAdminDataService', 'cmAlertService',
        function($scope, $http, cmAdminDataService, cmAlertService) {
            ...
            $scope.expandServiceDetails = function() {
                ...
                // TODO: This DOM manipulation code should not
                // be in controller.
                ...
                for (val in $scope.services) {
                    ...
                    // $('#service_row_' +
                    // service_row).animate({backgroundColor:
                    // 'transparent'}, "fast");
                    // $('#service_detail_row_' +
                    // service_row).animate({backgroundColor:
                    // 'transparent'}, "fast");
                ...
                if ($scope._visibility_flags[service]) {
                    // $('#service_row_' +
                    // service).animate({backgroundColor:
                    // 'transparent'}, "slow");
                    // $('#service_detail_row_' +
                    // service).animate({backgroundColor:
                    // 'transparent'}, "slow");
            ...
            $scope.performServiceAction = function($event, action) {
                // TODO: Ugly hack to let some stuff
                // passthrough. Fix.
...
cloudmanAdminModule
        .controller(
                ...
                [
                        ...
                        '$dialog',
                        'cmAdminDataService',
                        function($scope, $http, $dialog, cmAdminDataService) {
                            ...
                            $scope.toggleDetailView = function($event, fs) {
                                if (fs._showing_detail_popup) {
                                    ...
                                    cmAdminDataService.pauseDataService(); // Don't
                                    // let
                                    // updates
                                    // reset
                                    // file
                            ...
                            $scope.is_snapshot_in_progress = function(fs) {
                                return (fs.kind == "Volume" && fs.status == "Configuring")
                                        && (fs.snapshot_status != "" && fs.snapshot_status != null);
                            }

                            $scope.is_persistable_fs = function(fs) {
                                return ((fs.status === "Available" || fs.status === "Running") && (typeof (fs.from_snap) !== "undefined"
                                        && typeof (fs.DoT) !== "undefined" && fs.DoT === "Yes"));
                            }

                            $scope.is_resizable_fs = function(fs) {
                                return (fs.status === "Available" || fs.status === "Running")
                                        && (typeof (fs.kind) != "undefined" && fs.kind === "Volume");
                            }
...
cloudmanAdminModule.controller('GalaxyController', [ '$scope', '$http', 'cmAdminDataService', 'cmAlertService',
        function($scope, $http, cmAdminDataService, cmAlertService) {
            ...
            $scope.addAdminUsers = function($event, url) {
                var alert = cmAlertService.addAlert("Action Initiated", "info", 3000);
                $http({
                    method : 'POST',
                    url : url,
                    // TODO: DOM access in controller. Should be redone
                    data : $('#galaxy_admin_users_form').serialize(),
                    headers : {
                        'Content-Type' : 'application/x-www-form-urlencoded'
                    }
                }).success(function(data, status) {
            ...
            $scope.updateRespositoryUrl = function($event, url) {
                var alert = cmAlertService.addAlert("Action Initiated", "info", 3000);
                $http({
                    method : 'POST',
                    url : url,
                    // TODO: DOM access in controller. Should be redone
                    data : $('#galaxy_repository_form').serialize(),
                    headers : {
                        'Content-Type' : 'application/x-www-form-urlencoded'
                    }
                }).success(function(data, status) {
...
cloudmanAdminModule.controller('SystemController', [ '$scope', '$http', '$dialog', 'cmAdminDataService',
        'cmAlertService', function($scope, $http, $dialog, cmAdminDataService, cmAlertService) {
...
function FSPersistDialogController($scope, dialog, fs, cmAlertService) {
    ...
    $scope.confirm = function($event, url, result) {
        $http({
            ...
            url : url,
            // TODO: DOM access in controller. Should be redone
            data : $('#fs_persist_form').serialize(),
            headers : {
                'Content-Type' : 'application/x-www-form-urlencoded'
            }
        }).success(function(data, status) {
