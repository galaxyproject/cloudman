...
class CustomNonModelObjectMixin(object):
    """
    A viewset mixin to make it easier to work with non-django-model viewsets.

    Only the list_objects() and retrieve_object() methods need to be
    implemented. Create and update methods will work normally through DRF's
    serializers.
    """
    ...
    def list_objects(self):
        """
        Override this method to return the list of objects for list() methods.
        """
        pass
...
class CustomHyperlinkedRelatedField(relations.HyperlinkedRelatedField):
    """
    Allow arbitrary depth hyperlink fields to be built.
    ...
    names that are required for Django's ``reverse()`` to work. The values for
    each argument are obtained from the serializer context. It's modeled after
    drf-nested- routers' ``NestedHyperlinkedRelatedField``.
    """
...
class CustomHyperlinkedIdentityField(CustomHyperlinkedRelatedField):
    """
    ``CustomHyperlinkedRelatedField`` dedicated to creating identity links.

    It's simply copied from rest framework's
