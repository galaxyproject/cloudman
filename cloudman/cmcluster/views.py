...
from cmcluster import drf_helpers
from cmcluster import serializers
...
class ClusterViewSet(drf_helpers.CustomModelViewSet):
    ...
    def list_objects(self):
        ...
        return CloudManAPI().clusters.list()

    def get_object(self):
        """Get info about a specific cloud."""
        return CloudManAPI().clusters.get(self.kwargs["pk"])
...
class ClusterNodeViewSet(drf_helpers.CustomModelViewSet):
    ...
    def list_objects(self):
        cluster = CloudManAPI().clusters.get(self.kwargs["cluster_pk"])
        if cluster:
