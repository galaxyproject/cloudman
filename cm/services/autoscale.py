...
class Autoscale(Service):
    ...
    def status(self):
        ...
        if self.too_large():
            ...
            log.debug("Autoscaling DOWN: %s instance(s)" % num_instances_to_remove)
    ...
    def slow_job_turnover(self, threshold=60, num_queued_jobs=2):
        ...
        log.debug('Checking if slow job turnover: queued jobs: %s, avg runtime: %s' % (len(q_jobs['queued']), r_jobs_mean))
        ...
        if qstat_out != '': # else, the job queue is empty so just return
            ...
            for job in qstat_out:
                try:
                    ...
                    job_time_entered_state = datetime.datetime(*time.strptime(job.split()[1]+' '+job.split()[2], "%m/%d/%Y %H:%M:%S")[0:6])
                    if job_state == 'r':
                        running_jobs.append(self.total_seconds(now - job_time_entered_state))
                    elif job_state == 'qw':
                        queued_jobs.append(self.total_seconds(now - job_time_entered_state))
                except Exception, e:
    ...
    def get_num_instances_to_add(self):
        ...
        if len(self.app.manager.worker_instances)+num_instances_to_add < self.as_min:
            num_instances_to_add = int(self.as_min) - len(self.app.manager.worker_instances)
