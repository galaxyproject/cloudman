        ...
        if to_be_started and not self.state==service_states.RUNNING:
            ...
            if to_be_configured:
                ...
                if os.path.exists(self.app.path_resolver.galaxy_data) and not os.path.exists(psql_data_dir):
                    ...
                    log.error( "'%s' directory doesn't exist yet; will configure PostgreSQL later." % self.app.path_resolver.galaxy_data )
                    return False
                # Change ownership of just created directory
                if cont:
                    cont = misc.run( '%s -R postgres:postgres %s/pgsql' % ( paths.P_CHOWN, self.app.path_resolver.galaxy_data ), "Error changing ownership of just created directory", "Successfully changed ownership of just created directory" )
                # Initialize/configure database cluster
                if cont:
                    log.debug( "Initializing PostgreSQL database for Galaxy..." )
                    cont = misc.run( '%s - postgres -c "%s/initdb -D %s"' % (paths.P_SU, self.app.path_resolver.pg_home, psql_data_dir), "Error initializing Galaxy database", "Successfully initialized Galaxy database")
                    if cont:
                ...
                if cont:
                    ...
                    cont = misc.run( '%s - postgres -c "%s/pg_ctl -w -D %s -l /tmp/pgSQL.log -o \\\"-p %s\\\" start"' % (paths.P_SU, self.app.path_resolver.pg_home, psql_data_dir, self.psql_port), "Error starting postgres server as part of the initial configuration", "Successfully started postgres server as part of the initial configuration." )
                ...
                if cont:
                    ...
                    cont = misc.run('%s - postgres -c "%s/psql -p %s -c \\\"CREATE ROLE galaxy LOGIN CREATEDB\\\" "' % (paths.P_SU, self.app.path_resolver.pg_home, self.psql_port), "Error creating role for 'galaxy' user", "Successfully created role for 'galaxy' user" )
                # Create database for Galaxy, as galaxy user
                if cont:
                    log.debug( "Creating PostgreSQL database as 'galaxy' user..." )
                    cont = misc.run('%s - galaxy -c "%s/createdb -p %s galaxy"' % (paths.P_SU, self.app.path_resolver.pg_home, self.psql_port), "Error creating 'galaxy' database", "Successfully created 'galaxy' database")
                ...
                if cont:
                    ...
                    cont = misc.run('%s - postgres -c "%s/psql -p %s -c \\\"CREATE ROLE galaxyftp LOGIN PASSWORD \'fu5yOj2sn\'\\\" "' % (paths.P_SU, self.app.path_resolver.pg_home, self.psql_port), "Error creating role for 'galaxyftp' user", "Successfully created role for 'galaxyftp' user" )
                else:
    ...
    def check_postgres(self):
        ...
        if self.state==service_states.SHUTTING_DOWN or \
           self.state==service_states.SHUT_DOWN:
            ...
            dbs = commands.getoutput('%s - postgres -c "%s/psql -p %s -c \\\"SELECT datname FROM PG_DATABASE;\\\" "' % (paths.P_SU, self.app.path_resolver.pg_home, self.psql_port))
            ...
            log.warning("PostgreSQL data directory '%s' does not exist (yet?)" % self.app.path_resolver.psql_dir)
