...
import logging, threading, sys, os, os.path, time, subprocess, string, tempfile, re, traceback, shutil, random, pwd, grp, commands
...
class ConsoleManager( object ):
    ...
    def start( self ):
        self.mount_nfs( self.app.shell_vars['MASTER_IP'] )
    ...
    def mount_disk(self, master_ip, path):
        ...
        if not os.path.exists( path ):
            os.mkdir( path )
            # subprocess.call( "%s %s" % (P_MKDIR, path), shell=True )
    ...
    def unmount_nfs( self ):
        ...
        ret_code = subprocess.call( "umount -lf %s" % P_SGE_ROOT, shell=True )
        log.debug( "Process unmounting '%s' returned code '%s'" % (P_SGE_ROOT, ret_code) )
    ...
    def get_host_cert(self ):
        ...
        ret_code = subprocess.call( '%s - sgeadmin -c "ssh-keyscan -t rsa %s > %s"' % (P_SU, self.app.get_fqdn(), w_cert_file), shell=True )
        if ret_code == 0:
    ...
    def start_sge(self ):
        ...
        print >> f, sge_install_template % ( self.app.get_self_private_ip(), "", self.app.get_self_private_ip() )
        f.close()
        ...
        ret_code = subprocess.call( 'cd %s; ./inst_sge -x -noremote -auto %s' % (P_SGE_ROOT, SGE_config_file), shell=True )
        if ret_code == 0:
            self.sge_started = 1
...
#        if os.path.isfile( '/opt/sge/default/common/sgeexecd' ):
#            ret_code = subprocess.call( '/opt/sge/default/common/sgeexecd start', shell=True )
#            if ret_code == 0:
#                log.info ( "Successfully started SGE daemon; configuring users' SGE profiles..." )
#                f = open( "/etc/profile", 'a' )
#                f.write( "\nexport SGE_ROOT=/opt/sge" )
#                f.write( "\n. $SGE_ROOT/default/common/settings.sh" )
#                f.write( "\nexport PATH=/opt/galaxy/bin:/usr/postgres/8.3/bin/64:/galaxyTools/tools/bin:$GALAXY_HOME:/usr/gnu/bin:$PATH\n" )
#                f.close()
#            return ret_code
#        else:
#            log.error( "SGE daemon executable (/opt/sge/default/common/sgeexecd) not found!" )
            # TODO Need to enter 'repair' state or something...
...
class ConsoleMonitor( object ):
    def __init__( self, app):
        ...
        self.conn = comm.CMWorkerComm(self.app.get_instance_id(), self.app.shell_vars['MASTER_IP'])
        if self.app.TESTFLAG is True:
            log.debug("Attempted to get host cert, but TESTFLAG is set.")
        else:
            self.conn.setup()
    ...
    def send_alive_message( self ):
        msg = "ALIVE | %s | %s | %s | %s | %s" % (self.app.get_self_private_ip(), 
                                                    self.app.get_self_public_ip(), 
                                                    self.app.get_zone(), 
                                                    self.app.get_type(), 
                                                    self.app.get_ami())
        self.conn.send(msg)
    ...
    def send_node_ready(self):
        ...
        msg_body = "NODE_READY | %s | %s" % (self.app.get_instance_id(), num_cpus)
        log.debug( "Sending message '%s'" % msg_body )
        log.info( "Instance '%s' done configuring itself, sending NODE_READY." % self.app.get_instance_id() )
        self.conn.send(msg_body)
        
    def send_node_shutting_down(self):
        msg_body = "NODE_SHUTTING_DOWN | %s | %s" % (self.app.manager.worker_status, self.app.get_instance_id())
        log.debug( "Sending message '%s'" % msg_body )
        self.conn.send(msg_body)
    ...
    def handle_message(self, message):
        if message.startswith("RESTART"):
            ...
            self.app.shell_vars['MASTER_IP'] = m_ip
            self.app.manager.unmount_nfs()
            self.app.manager.mount_nfs( self.app.shell_vars['MASTER_IP'] )
