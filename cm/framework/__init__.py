...
import helpers
# This adds the routes url_for function to the helpers bundle that gets
# sent for template generation.
...
class WebApplication(base.WebApplication):
    def __init__(self, galaxy_app, session_cookie='galaxysession'):
        ...
        self.set_transaction_factory(lambda e: UniverseWebTransaction(
            e, galaxy_app, self, session_cookie))
        # Mako support
...
class FormBuilder(object):
    ...
    def add_input(self, type, name, label, value=None, error=None, help=None, use_label=True):
        self.inputs.append(
            FormInput(type, label, name, value, error, help, use_label))
        return self
